...
************/
  ...
  char *filename5 = Form("p%c%i_%iGeVParticleList.txt",ty,jobID,modSNN);
...
//      cout<<-log(tan(Theta/2))<<" vs "<<0.5*log((p+pz)/(p-pz))<<"      and      "<<Theta<<" vs "<<Theta1<<endl;
      ...
      if (event==0){
        if (N =='y') pz*=(-1);
        out4<<ind<<"\t"<<NameOfParticle<<"\t"<<STATUS<<"\t"<<KFID<<"\t"<<mpP<<"\t"<<px<<"\t"<<py<<"\t"<<pz<<"\t"<<E<<"\t"<<m<<endl;
        if (N =='y') pz*=(-1);
      }
      if (CUT=='y'){
        cuttype=rapidity;
    ...
    *****************************************/
    ...
    ccEta-=cetapis;
    if (ETEta!=0){
      hETEta->Fill(ETEta-ETetapis);
      hPTEta->Fill(PTEta-PTetapis);
    ...
    ccOmega-=comegapis;
    if (ETOmega!=0){
      hETOmega->Fill(ETOmega-ETomegapis);
      hPTOmega->Fill(PTOmega-PTomegapis);
