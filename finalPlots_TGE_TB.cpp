...
// main function:
int finalPlots_TGE_TB(){

	Double_t f_pi=1.588;
	Double_t f_piErr=0.029;
	Double_t f_K=1.8; //fk- = 1.5+-0.1        fk+ = 0.5 +- 0.05
	Double_t f_KErr=0.1;
	Double_t f_p=1.26; //add in for energy dependance
	Double_t f_pErr=0.26;
	Double_t f_Lam=1.08;//.4373948
	Double_t f_LamErr=0.51;//.12
	...
	*/
	...
	f = new TFile("crossCheckGraphs_TB.root","RECREATE");
	for(int j=0; j<=269+70; j++){// for 270 rows below header in the data file in1
								// and 70 rows below header in data file in2
		//for(int i=0; i<46; i++){ DEBUG
		if (j == 0)
		{
			//ifstream in1;
		...
		if(j>=271) in1 >> 	skipContent; //DEBUG
		...
		if(partIndex(part)==0 || partIndex(part)==1){ //PIONS
			dETdEtaSum[centIndex(cent)][enIndex(collEn)]
			 += (f_pi)*dETdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)]; //originally *(3.0/2.0)
			...
			dETdEtaSum_errSq[centIndex(cent)][enIndex(collEn)]
			 += (dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)])*
			    (1/f_pi)*(f_piErr)*(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)]); //originally both *(3.0/2.0)
			dETdEtaSum_err[centIndex(cent)][enIndex(collEn)]
			 =  TMath::Sqrt(dETdEtaSum_errSq[centIndex(cent)][enIndex(collEn)]);
			dETdEtaSum_err[centIndex(cent)][enIndex(collEn)]
			 =  (dETdEtaSum[centIndex(cent)][enIndex(collEn)])
			 *  TMath::Sqrt(((dETdEtaSum_err[centIndex(cent)][enIndex(collEn)]/dETdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)])*(dETdEtaSum_err[centIndex(cent)][enIndex(collEn)]/dETdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)])));
			...
			dNchdEtaSum_err[centIndex(cent)][enIndex(collEn)]
			 =  (dNchdEtaSum[centIndex(cent)][enIndex(collEn)])
			 *  TMath::Sqrt(((dNchdEtaSum_err[centIndex(cent)][enIndex(collEn)]/dNchdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)])*(dNchdEtaSum_err[centIndex(cent)][enIndex(collEn)]/dNchdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)])));
		}
		else if(partIndex(part)==2 || partIndex(part)==3){ //KAONS
			dETdEtaSum[centIndex(cent)][enIndex(collEn)]
			 += (f_K)*dETdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)];
			dETdEtaSum_errSq[centIndex(cent)][enIndex(collEn)]
			 += (1/f_K)*(f_KErr)*(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)])*
				(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)]);
		...
		else if(partIndex(part)== 4 || partIndex(part)== 5){ //PROTONS
			dETdEtaSum[centIndex(cent)][enIndex(collEn)]
			 += f_p*dETdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)];
			dETdEtaSum_errSq[centIndex(cent)][enIndex(collEn)]
			 += (1/f_p)*(f_pErr)*(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)])*
				(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)]);
			// ^ since num of k0_s ~ num of k0_l = num of k+ or k-
				// also, num of p ~ num of pbar ~ num of n or nbar
			dETdEtaSum_err[centIndex(cent)][enIndex(collEn)]
		...
		else if(partIndex(part)==6 || partIndex(part)==7){ //LAMBDAS
			dETdEtaSum[centIndex(cent)][enIndex(collEn)]
			 += (f_Lam)*dETdEta[centIndex(cent)][enIndex(collEn)][partIndex(part)];
			dETdEtaSum_errSq[centIndex(cent)][enIndex(collEn)]
			 += (1/f_Lam)*(f_LamErr)*(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)])*
				(dETdEtaErr[centIndex(cent)][enIndex(collEn)][partIndex(part)]);
			// ^ since num of k0_s ~ num of k0_l = num of k+ or k-
				// also, num of p ~ num of pbar ~ num of n or nbar
			dETdEtaSum_err[centIndex(cent)][enIndex(collEn)]
...
/******************************************************************************
NOTE: this is where the pi0 and eta are added. above (line 156)
