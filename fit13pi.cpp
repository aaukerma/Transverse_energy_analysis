...
TFile* myFile = TFile::Open("HEPData-ins1797443-v1-Table_1.root");

TDirectory* dir = myFile->GetDirectory("Table 1");
TH1D* h=(TH1D* )myFile->Get("Table 1/Hist1D_y1");
...
Double_t avgET=0.0;
		...
		EXPO2->SetParNames("A2","B2","C2","mass","type");
			EXPO2->SetParLimits(0,60,63.83);//A
		EXPO2->SetParLimits(1,-4.0,0.313);//B
		EXPO2->SetParLimits(2,0.15,0.2232);//C
...
Double_t dETdEtaTErrE2 = dETdEtaLErrE2+dETdEta_d_errE2+dETdEtaRErrE2;
	...
	avgET=(dETdEtaTotalE+dETdEtaTotalE2)/2.0;
		...
		avg <<collEn << "\t"
		    ...
		    << avgETy_err <<  "\t"
		    << avgETyfitErr <<  "\n";
	string imgPathAndName = "./Table1/13TeVPi.png";
