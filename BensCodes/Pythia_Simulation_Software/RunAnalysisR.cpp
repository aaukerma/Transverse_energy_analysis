...
int RunAnalysisR(){
  Int_t NUMSNN = 8; //CHANGE TO INCLUDE HIGHER SNNS ie (8 = 7.7 to 200, 10 = 7.7 to 5020)
  Int_t IDNUM=1;
  ...
  c{"AVG","S","N"};
  *****************************************************************************/
  vector<vector<vector<dat>>> Stuff(NUMSNN, vector<vector<dat>>(2, vector<dat>(16)));
  vector<vector<vector<d>>> StuffTXT(NUMSNN, vector<vector<d>>(2, vector<d>(24)));
  vector<vector<vector<d>>> TOTs(NUMSNN, vector<vector<d>>(2, vector<d>(9)));
  vector<dat> Jdata(NUMSNN);

  ofstream outfile;
  outfile.open("AnalysisOutfile.txt");
  //VALUEs for each histo
  vector<string> particles={"ALL","PiP","PiM","Pi0","KaP","KaM","K0L","K0S","ETA","OMG","La0","LB0","PRO","NEU","PRB","NEB","OMm","XI0","XIM","SIP","SIM","SI0"};
  ...
  cin>>IDNUM;
  outfile<<"RunNumber: "<<IDNUM<<" ";
  cout<<"pp or pA?:"<<endl;
  cin>>ty;
  if (ty=='A') outfile<<"Angantyr Au+Au\n";
  else if (ty=='a') outfile<<"Anganyr He+He\n";
  else if (ty=='p') outfile<<"p+p\n";
  cout<<"preparing data for run: "<<IDNUM<<endl<<endl;
  for (int x=0;x<NUMSNN;x++){
    double TET;
    double TPT;
    int num;
    double et;
    ...
    double pet;
    double ppt;
    char* temp = Form("Data/root/p%c%i_%iGeVoutfile.root",ty,IDNUM,SNN[x]);
    char* temptxt = Form("Data/outfile/p%c%i_%iGeVoutfile.txt",ty,IDNUM,SNN[x]);
    outfile<<"Energy: "<<SNNACTUAL[x]<<"\nParticle: \tNumber'o'Particles:\n";
    TFile f(temp);
    f.Open(temp);
    ifstream f2;
    f2.open(temptxt);
    ...
    TH1F* hETother= (TH1F*)f.Get("hETother")->Clone();
    ...
    TH1F* hptother= (TH1F*)f.Get("hptother")->Clone();

    TH1F* hJHisto= (TH1F*)f.Get("hJHisto")->Clone();
    cout<<"Histos Loaded\n";
    f.Close();
    ...
    b=0;
    for (int bop=1;bop<16;bop++){
      outfile<<particles[bop]<<"\t"<<Stuff[x][a][bop].entries<<endl;
    }
