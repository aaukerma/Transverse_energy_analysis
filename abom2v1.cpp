...
TFile* myFile = TFile::Open("HEPData-ins885104-v1-Table_2.root");

TDirectory* dir = myFile->GetDirectory("Table 2");
TH1D* h=(TH1D* )myFile->Get("Table 2/Hist1D_y1");
...
Double_t avgET=0.0;
		...
		EXPO->SetParameters(.1,0.1,mass,type);
		EXPO->SetParNames("Ae","Be","mass","type");
		EXPO->SetParLimits(0,.09,2);
		...
		EXPO2->SetParNames("A2","B2","C2","mass","type");
			EXPO2->SetParLimits(0,4,4.186);//A
		EXPO2->SetParLimits(1,-0.4,-0.3541);//B
		EXPO2->SetParLimits(2,0.15,0.2379);//C
	...
	avgET=(dETdEtaTotalE+dETdEtaTotalE2)/2.0;
		...
		avg <<collEn << "\t"
		 << mass << "\t"
		    ...
		    << avgETy_err <<  "\t"
		    << avgETyfitErr <<  "\n";
	string imgPathAndName = "./Table2/"+histoName+".png";
