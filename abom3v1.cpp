...
// main function:
int abom3v1(){
std::ofstream avg ("avgfitResults2.dat", std::ofstream::app);
TFile* myFile = TFile::Open("HEPData-ins885104-v1-Table_3.root");

TDirectory* dir = myFile->GetDirectory("Table 3");
TH1D* h=(TH1D* )myFile->Get("Table 3/Hist1D_y1");

TF1* EXPO;
TF1* EXPO2;
TCanvas* c1;
TGraphAsymmErrors *g = (TGraphAsymmErrors *)dir->Get("Graph1D_y1");
g->Draw();
...
Double_t avgET=0.0;
		...
		EXPO->SetParameters(0.1,0.1,mass,type);
		EXPO->SetParNames("Ae","Be","mass","type");
		//EXPO->SetParLimits(0,0.01,.02);
		//EXPO->SetParLimits(1,0.4,0.5);
		EXPO->SetLineColor(kBlack);
		EXPO->FixParameter(3,type);
		EXPO->FixParameter(2,mass);
		
		EXPO2->SetParameters(.01,.01,.01,mass,type);
		EXPO2->SetParNames("A2","B2","C2","mass","type");
			EXPO2->SetParLimits(0,40,43);//A
		EXPO2->SetParLimits(1,-4.0,0.0514);//B
		EXPO2->SetParLimits(2,0.15,0.1775);//C
	...
	avgET=(dETdEtaTotalE+dETdEtaTotalE2)/2.0;
		...
		avg <<collEn << "\t"
		 << mass << "\t"
		    ...
		    << avgETy_err <<  "\t"
		    << avgETyfitErr <<  "\n";
	string imgPathAndName = "./Table3/"+histoName+".png";
