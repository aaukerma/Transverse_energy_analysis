...
void fastMcProduction200_list(Int_t nev = 300) {
...
*******************************************************************************/
   cout<<"Number of nprim particles: "<<nprim<<endl;
   cout<<"number of particle entries: "<<pnum<<endl;
   cout<<"Number of events: "<<nev<<endl;
   out<<"Event No. "<<iev<<endl;
   out<<"Number of Entries from call stack->GetNprimary: "<<nprim<<endl;
   out<<"Number of Entries from call ALLPART->GetEntries: "<<pnum<<endl;
   out<<"Index\tPDG\tname\tStatus\tIsPrimary\tMother1\tMother2\tDaughter1\tDaughter2\tP[GeV]\tE[GeV]\tParticleType\n";
   for (int partnum=0;partnum<=(nprim-1);partnum++){
     ...
     Int_t mpP2 = part->GetSecondMother();
     Int_t stat = part->GetStatusCode();
     Double_t mpL = 0;
     Double_t partE;
     Int_t nobby=15;
     Bool_t PRIM = part->IsPrimary();
     string particletype;
     /******************************************************************************
     ...
     ******************************************************************************/
            if ((PDG==3122)||(PDG==2212)||(PDG==2112)){
              ...
              partE = Ei * sin(Theta);
              particletype="Meson/Other";
            }
     out<<partnum<<"\t"<<PDG<<"\t"<<name<<"\t"<<stat<<"\t"<<PRIM<<"\t"<<mpP<<"\t"<<mpP2<<"\t"<<mpartD<<"\t"<<mpartD2<<"\t"<<P<<"\t"<<partE<<"\t"<<particletype<<endl;
     if (rapidity<.1){
